...
connection.on('error', (err) => {
	// Handle disconnection, reconnect, or other appropriate action
	if (err.code === 'PROTOCOL_CONNECTION_LOST') {
			console.log('Database connection lost. Reconnecting...');
			// The connection pool will automatically attempt to acquire a new connection
			// after the acquireTimeout period. You don't need to manually trigger a reconnection.
	} else {
			throw err; // Unhandled errors
	}
});
...
const transporter = nodemailer.createTransport({
	...
	secure: true,
	auth: {
		user: 'smithzachwilliam@gmail.com',
		pass: 'vhnh onfc mzln xhcq'
	}
});
// Initialize express
const app = express();
...
app.get(['/', '/login'], (request, response) => isLoggedin(request, () => {
	...
	// Create CSRF token
	const user = { username: response.account_username };
	...
	if (username && password) {
		...
		connection.query('SELECT * FROM accounts WHERE username = ? AND password = ?', [username, hashedPassword], (error, accounts) => {
			...
			if (accounts.length > 0) {
				...
				request.session.account_tier = accounts[0].tier;
...
app.post('/register', (request, response) => init(request, settings => {
	...
	let token = request.body.token;
	let registered = new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, -1).split('.')[0];
	// Get client IP address
	let ip = request.headers['x-forwarded-for'] || request.socket.remoteAddress;
	// Default role
	let role = 'Member';
	...
	if (username && password && email) {
		...
		connection.query('SELECT * FROM accounts WHERE username = ? OR email = ?', [username, email], (error, accounts, fields) => {
			...
			if (accounts.length > 0) {
				...
				connection.query('INSERT INTO accounts (username, password, email, activation_code, registered, role, ip) VALUES (?, ?, ?, ?, ?, ?, ?)', [username, hashedPassword, email, activationCode, registered, role, ip], () => {
				...
				// Insert account
				connection.query('INSERT INTO accounts (username, password, email, activation_code, role, registered, ip) VALUES (?, ?, ?, "activated", ?, ?, ?)', [username, hashedPassword, email, role, registered, ip], (error, result) => {
